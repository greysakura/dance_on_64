...
if __name__ == "__main__":
    ...
    for query_i in range(len(query_img_dir_list)):
        ...
        num_negatives = len(DQE_negative)
        DQE_data = list(DQE_positive)
        DQE_data.extend(DQE_negative)

        #### 2014/11/26 adding all SV failed into negatives
        if len(SV_IDF_score) < num_for_SV:
            SV_failed = TF_IDF_norm_matrix[SV_reranking_Idx_list[len(SV_IDF_score):num_for_SV], :].tolist()
            DQE_data.extend(SV_failed)
            num_negatives += len(SV_failed)

        print 'number of positives: ', len(DQE_positive)
        print 'number of negatives: ', num_negatives
        unbalance_ratio = len(DQE_positive)/np.float64(len(DQE_negative))
        DQE_label = [1]*len(DQE_positive) +  [0]*num_negatives
