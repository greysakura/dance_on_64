...
if __name__ == "__main__":
    ...
    for query_i in range(len(query_wanna_check)):
        #### make a new dir
        tmp_SV_output_dir = SV_output_dir + query_wanna_check[query_i] + '/'
        try:
            os.stat(tmp_SV_output_dir)
        except:
            os.mkdir(tmp_SV_output_dir)


        ##############
        tmp_good_dir = ground_truth_dir = top_dir + 'ground_truth_file/' + query_wanna_check[query_i] + 'good.txt'
        tmp_ok_dir = ground_truth_dir = top_dir + 'ground_truth_file/' + query_wanna_check[query_i] + 'ok.txt'
        tmp_junk_dir = ground_truth_dir = top_dir + 'ground_truth_file/' + query_wanna_check[query_i] + 'junk.txt'
        ...
        result_file = open(query_goto_dir + query_wanna_check[query_i] + 'query.txt','r')
        ...
        img_query = cv2.imread(query_goto_dir + query_wanna_check[query_i] + 'query.jpg')
        cv2.imwrite(tmp_SV_output_dir + 'query_' + query_wanna_check[query_i] + '.jpg', img_query)
        ...
        for i in range(len(top_several_results)):
            ...
            if top_results_judge[0,i] == 1:
                ...
                minGoodMatch = 10
                ###
                cv2.rectangle(tmp_img,(0,0),(tmp_img.shape[1],tmp_img.shape[0]),(0,255,0),thickness=20)
                ###
                ...
                cv2.rectangle(tmp_img,(0,0),(tmp_img.shape[1],tmp_img.shape[0]),(255,0,0),thickness=20)

            cv2.imwrite(tmp_SV_output_dir + 'SV_result_'+ str(i+1) + '_' + top_several_results[i] + '.jpg', tmp_img)
